...
from gnn_example.util.constants import TEST_SIZE, PROJECT_ROOT


def load_data(file_path: Path = Path(f"{PROJECT_ROOT}/data/sample-dataset.csv"), sampling_ratio=1.0):
...
def save_graph_data(data: Data, name: str = None, path: str = None):
    ...
    if path is not None:
        ...
        file_path = Path(f"{PROJECT_ROOT}/data/graph/{name}")
